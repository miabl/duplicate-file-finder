...
//
// RETURNS THE LIST OBJECT CONTAINING DUPLICATES OF THE FILE, OR NULL IF NO DUPLICATES ARE FOUND
  ...
  if (list != NULL && found_list != NULL) {
    for(int i = 0; i < found_list->count; i++) {
      ...
      if(statinfo.st_ino == info->inode) {
        info->relative_filepaths = realloc(info->relative_filepaths, sizeof(info->relative_filepaths[0]) * (info->filecount+1));
        info->relative_filepaths[info->filecount] = rel_path;
        info->absolute_filepaths = realloc(info->absolute_filepaths, sizeof(info->absolute_filepaths[0]) * (info->filecount+1));
    ...
    // IF THE INODE ISN'T FOUND, ADD THE FILEINFO STRUCT TO THE FILEINFO ARRAY
      list->fileinfo = realloc(list->fileinfo,
                               sizeof(list->fileinfo[0]) * (list->count + 1));
